...
## 3. Важные аспекты игры

### Динамическая сложность
Сложность игры регулируется параметром `Game.stats.badge`.
Сложность влияет на:
1.  Увеличение количества студентов за игровой день.
2.  Повышение размера штрафа за ошибку.
3.  После 10-го уровня добавляется механика проверки рюкзаков.

### Нормальное распределение студентов
Поток студентов распределяется по рабочему дню согласно нормальному распеределению.

### Валидация ID пропуска
ID генерируется по формуле: `[3 цифры] + [Фамилия] + [1-я буква Имени] + [2 буквы Отчества]`.
С вероятность в 10% генератор намеренно использует имя *противоположного пола*, чтоб поймать игрока на ошибке.
